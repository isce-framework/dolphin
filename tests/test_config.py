...
from dolphin.workflows.config import get_workflow_yaml_path
...
def test_yaml_stack():
    ...
    min_data = dolphin.workflows.config.load_workflow_yaml(
...
def test_yaml_save(tmp_path):
    ...
    dolphin.workflows.config.save_yaml(temp_file, min_data)
    y = YAML()
...
def test_dolphin_cfg_section():
    ...
    cfg_augmented = dolphin.workflows.config.add_dolphin_section(min_data)
