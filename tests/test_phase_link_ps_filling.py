...
def test_failing_ps_size():
    ...
    # data = np.load(Path(__file__).parent / "data/ps-fix/failing_data.npz")
    # # assert cpx_phase == data["cpx_phase"]
    # temp_coh = data["temp_coh"]
    # slc_stack = data["slc_stack"]
    # strides = data["strides"]
    # avg_mag = data["avg_mag"]
    # ps_mask = data["ps_mask"]
    # reference_idx = data["reference_idx"]

    # # np.all(np.where(~np.isnan(avg_mag))[0] == np.where(ps_mask)[0])
    # rows, cols = np.where(~np.isnan(avg_mag))
    # t, y, x = np.where(np.isnan(slc_stack))
    # print(rows.shape)
    # print(t.shape)

    # np.savez_compressed(
    #     "failing_data_idxs.npz",
    #     # strides=strides,
    #     # avg_mag=avg_mag,
    #     not_nan_rows=rows,
    #     not_nan_cols=cols,
    ...
    #     # reference_idx=reference_idx,
    #     # temp_coh=temp_coh,
    #     # slc_stack=0 * slc_stack,
    # )
    # fill_ps_pixels(
    #     cpx_phase=data["cpx_phase"],
    #     temp_coh=data["temp_coh"],
    #     slc_stack=data["slc_stack"],
    #     strides=data["strides"],
    #     avg_mag=data["avg_mag"],
