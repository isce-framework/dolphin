...
def filtering(
    ...
    X_ = np.hstack(( np.reshape(X1_, (nrow*ncol, 1)) , np.reshape(X2_, (nrow*ncol, 1)) ) )
    ...
    for k in range(NW[1], NE[1]+1):
        n_zeros = np.count_nonzero(unw_ifg_interp[0:NE[0]+1, k] == 0) # count zeros in North direction
        EV_fill[0:n_zeros, k] = EV_fill[n_zeros,k]
    for k in range(SW[1], SE[1]+1):
        n_zeros = np.count_nonzero(unw_ifg_interp[SW[0]+1:, k] == 0) # count zeros in South direction
        EV_fill[-n_zeros:, k] = EV_fill[-n_zeros-1, k]
    for k in range(NW[0], SW[0]+1):
        n_zeros = np.count_nonzero(unw_ifg_interp[k, 0:NW[1]+1] == 0) # count zeros in North direction
        EV_fill[k, 0:n_zeros] = EV_fill[k, n_zeros]
    for k in range(NE[0], SE[0]+1):
        n_zeros = np.count_nonzero(unw_ifg_interp[k, SE[1]+1:] == 0) # count zeros in North direction
    ...
    for k in range(SW[1], SE[1]+1):
        n_zeros = np.count_nonzero(unw_ifg_interp[SW[0]+1:, k] == 0) # count zeros in South direction
        Reflect_fill[-n_zeros:, k] = np.flipud(EV_fill[-n_zeros-n_zeros:-n_zeros, k])
    for k in range(NW[0], SW[0]+1):
        n_zeros = np.count_nonzero(unw_ifg_interp[k, 0:NW[1]+1] == 0) # count zeros in North direction
        Reflect_fill[k, 0:n_zeros] = np.flipud(EV_fill[k, n_zeros:n_zeros+n_zeros])
    for k in range(NE[0], SE[0]+1):
        n_zeros = np.count_nonzero(unw_ifg_interp[k, SE[1]+1:] == 0) # count zeros in North direction
        Reflect_fill[k, -n_zeros:] = np.flipud(EV_fill[k, -n_zeros-n_zeros:-n_zeros])

    Reflect_fill[0:NW[0], 0:NW[1]] = np.flipud(Reflect_fill[NW[0]:NW[0]+NW[0], 0:NW[1]]) # upper left corner area
    Reflect_fill[0:NE[0], NE[1]+1:] = np.fliplr(Reflect_fill[0:NE[0], NE[1]+1-(ncol-NE[1]-1):NE[1]+1]) # upper right corner area
    Reflect_fill[SW[0]+1:, 0:SW[1]] = np.fliplr(Reflect_fill[SW[0]+1:, SW[1]:SW[1]+SW[1]]) # lower left corner area
    Reflect_fill[SE[0]+1:, SE[1]+1:] = np.flipud(Reflect_fill[SE[0]+1-(nrow-SE[0]-1):SE[0]+1, SE[1]+1:]) # lower right corner area

    # 2D filtering with Gaussian kernel
    #wavelength_cutoff: float = 50*1e3,
    #dx: float = 30,
    cutoff_value = 0.5 # 0 < cutoff_value < 1
    sigma_f = 1/wavelength_cutoff/np.sqrt(np.log(1/cutoff_value)) # fc = sqrt(ln(1/cutoff_value))*sigma_f
