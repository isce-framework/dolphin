...
class BaseStack(BaseModel):
    ...
    def compressed_slc_file_list(self) -> list[Filename]:
        ...
        return [f for f, is_comp in zip(self.file_list, self.is_compressed, strict=False) if is_comp]

    @property
    def real_slc_file_list(self) -> list[Filename]:
        """List of real SLCs for this ministack."""
        return [
            f for f, is_comp in zip(self.file_list, self.is_compressed, strict=False) if not is_comp
        ]
...
class MiniStackInfo(BaseStack):
    ...
    def get_compressed_slc_info(self) -> CompressedSlcInfo:
        ...
        for f, d, is_comp in zip(self.file_list, self.dates, self.is_compressed, strict=False):
            if is_comp:
